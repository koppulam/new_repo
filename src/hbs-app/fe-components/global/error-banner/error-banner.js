 ...
 * @module error-banner
 * @version 1.0.0
 * @since Thu Oct 25 2018
 */

// dependencies
import { findFirst, addClass, removeClass } from 'lib/dom/dom-util';
import { getUrlParameter } from 'lib/utils/filters';

import './error-banner.hbs';
 ...
 * @type {string}
 */
export const componentReference = 'app-js__error-banner';

/**
 * The style definition of the component.
 * @type {string}
 */
export const styleDefinition = 'error-banner';
 ...
 */
function createErrorbannerInstance() {
    /**
     * Component instance.
     * @type {Object}
     */
     ...
     */
    function init() {
        const fallback = getUrlParameter('fallback');

        if (fallback) {
            removeClass(findFirst('.error-banner'), 'hide');
        }
    }

    /**
     * Close Error banner on click of close icon
     * @param {object} data event data
     * @returns {void}
     */
    function closeErrorBanner() {
        addClass(findFirst('.error-banner'), 'hide');
    }
    ...
    function addListeners() {
        ...
        instance.addEventListener('click', closeErrorBanner, findFirst('.error-banner--close'));
    }
...
registerComponent(componentReference, createErrorbannerInstance);

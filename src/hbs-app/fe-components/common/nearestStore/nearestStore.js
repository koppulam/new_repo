...
// dependencies
import './nearestStore.hbs';
import './nearestStore.scss';
 ...
 * @type {string}
 */
export const componentReference = 'app-js__nearestStore';

/**
 * The style definition of the component.
 * @type {string}
 */
export const styleDefinition = 'nearestStore';
 ...
 */
function createNearestStoreInstance() {
    /**
     * Component instance.
     * @type {Object}
     */
     ...
     */
    function init() {
        if (singleRefs.nearestStore && navigator.geolocation) {
            navigator.geolocation.getCurrentPosition((position) => {
                ...
                const apiConfig = objectPath.get(state, 'aem.storeLocatorConfig', {});
                const apiUrl = objectPath.get(window, 'tiffany.apiUrl', []);

                if (apiUrl[apiConfig.url]) {
                    apiConfig.url = apiUrl[apiConfig.url];
                }

                if (Object.keys(apiConfig).length > 0) {
                    apiConfig.payload.latitudeInput = coordinates.lat;
                    apiConfig.payload.longitudeInput = coordinates.lng;

                    ApiUtils.makeAjaxRequest(
                        apiConfig,
                        res => {
                            if (res.resultDto && res.resultDto.length > 0) {
                                singleRefs.nearestStore.innerHTML = objectPath.get(res.resultDto[0], 'store.storeName', '');
                            }
                        },
                        err => {
                            // error in ajax call
            ...
            // user opted not to share the location
            // code TODO
        }
    }
...
registerComponent(componentReference, createNearestStoreInstance);
